...
*/
        ...
        if($sProjid != "HORMONES"){
          $query ="SELECT PUV.visit_id, PUV.group_id, PUV.schedule_date,
            ...
            VL.visit_name, PUV.visit_status, VL.visit_day_before, VL.visit_day_after, VS.status_name, PG.proj_id_name, 
            '' as e2_id,
            '' as e2_result,
            '' as t_id,
            '' as t_result,
          ...
          LEFT JOIN p_visit_status VS ON (PUV.visit_status=VS.status_id)
          LEFT JOIN p_project_group PG ON (PG.proj_id = PUV.proj_id and PG.proj_group_id = PUV.group_id)
          ...
          $query = "SELECT
            ...
            VS.status_name,
            PG.proj_id_name,
            p_data_e2.data_id as e2_id,
            p_data_e2.data_result as e2_result,
            p_data_t.data_id as t_id,
            p_data_t.data_result as t_result,
            g_list.group_name
          FROM
            ...
            LEFT JOIN p_visit_status VS ON ( PUV.visit_status = VS.status_id )
            LEFT JOIN p_project_group PG ON ( PG.proj_id = PUV.proj_id AND PG.proj_group_id = PUV.group_id )
            left join p_data_result p_data_e2 on(PUV.uid = p_data_e2.uid and PUV.visit_date = p_data_e2.collect_date and p_data_e2.collect_time <= '00:05:00' and p_data_e2.data_id = 'E2') 
            left join p_data_result p_data_t on(p_data_t.uid = PUV.uid AND p_data_t.collect_date = PUV.visit_date AND p_data_t.collect_time <= '00:05:00' AND p_data_t.data_id = 'T' )
            left join p_data_result h_status on(h_status.uid = p_data_e2.uid and h_status.collect_date = p_data_e2.collect_date and h_status.collect_time = p_data_e2.collect_time and h_status.data_id = 'hormo_status')
            left join i_project_filter_list g_list on(g_list.seq_no = h_status.data_result and g_list.proj_id = 'HORMONES')
            WHERE PUV.uid =? AND PUV.proj_id =? AND PUV.visit_status <> 'C'
            ORDER BY PUV.schedule_date, VL.visit_order
            ";
        }
          ...
          $prev_visit_date = "0000-00-00";
            ...
            $row["proj_id_name"], $row["group_name"], $row["e2_result"], $row["t_result"]);
...
$window_period_before, $window_period_after, $is_visit_next, $proj_id_name, $hormone_status, $e2, $t)
{
  // echo $hormone_status."/".$e2."/".$t;
  global $proj_auth;
  ...
  if(isset($proj_auth['allow_schedule'])){
    ...
    $txt_visit_date = "
     <div class='fl-wrap-row'>
       <div class='fl-wrap-col fl-mid pbtn pbtn-blue pw100 ph30 pround5 b-txt view-form vd-$row_no' data-visitid='$visit_id' data-visitdate='$visit_date' data-groupid='$group_id'>
  ...
  if($proj_id == "SDART"){
    ...
    $txt_row = "
      <div class='fl-wrap-row  visit-row  $classRow p-row ptxt-s10 pt-1 px-1' data-row='$row_no' data-visitid='$visit_id' data-sdate='$schedule_date' data-vdate='$visit_date' data-vstatus='$visit_status' >
        ...
        <div class='fl-fix w-120 '>$proj_id_name </div>
        <div class='fl-wrap-col w-120'>
        ...
        if($visit_date != '0000-00-00' && $visit_id != "D0"){
          $txt_row .=  "<div class='fl-fill fl-auto '> <button class='pbtn-ok consent_rep' data-uid='$uid' data-visit='$visit_date'>Consent Report</button></div>";
        }else{
          $txt_row .=  "<div class='fl-fill fl-auto '> <button class='pbtn consent_rep' disabled='disabled'>No Consent Report</button></div>";
        }
  ...
  $txt_row = "
    <div class='fl-wrap-row  visit-row  $classRow p-row ptxt-s10 pt-1 px-1' data-row='$row_no' data-visitid='$visit_id' data-sdate='$schedule_date' data-vdate='$visit_date' data-vstatus='$visit_status' >
      ...
      <div class='fl-fix w-5 h-20 hormones-lab' style='display: none;'></div>
      <div class='fl-fix w-20 h-20 pbadge-blue hormones-lab' style='display: none;'>
        E2
      </div>
      <div class='fl-fix w-30 h-20 pbadge-white hormones-lab' style='display: none;'>
        $e2
      </div>
      <div class='fl-fix w-5 h-20 hormones-lab' style='display: none;'></div>
      <div class='fl-fix w-20 h-20 pbadge-blue hormones-lab' style='display: none;'>
      ...
      <div class='fl-fix w-5 hormones-lab' style='display: none;'></div>
    </div>
  ";
}//end else
  return $txt_row;
...
$(document).ready(function(){
  ...
  var projid = $("#div_visit_schedule_data").data("projid");
  if(projid == "HORMONES"){
    $(".hormones-lab").show();
  }
  ...
  $('#div_visit_schedule_data').off('click');
    ...
    view_visit_form_list(visitid, visitdate, groupid);
	});
...
$('.consent_rep').on("click",function(){
  //alert("report consent");
  let sUid = $(this).attr('data-uid');
  let sVisit = $(this).attr('data-visit');
  let tmp_age = $('#consent_age').val().split(" ");
 //alert("AGE : "+tmp_age[0]);
 //alert("UID : "+sUid);
  if(tmp_age[0] < 18){
    window.open("http://192.168.100.48/SDART/sdart_less18.php?UID="+sUid+"&VS="+sVisit , '_blank');
  }
  if(tmp_age[0] > 17){
    window.open("http://192.168.100.48/SDART/sdart_over18.php?UID="+sUid+"&VS="+sVisit , '_blank');
  }
