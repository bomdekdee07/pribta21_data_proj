...
if($stmt->execute()){
  ...
  while ($stmt->fetch()) {
		...
		$aPInfo[$uid]["en_sname"] = $en_sname;
	  	...
	  	$aPInfo[$uid]["remark"] = urlencode($remark);
		...
		$aPOdata[$uid]["en_fname"] = $en_fname;
		$aPOdata[$uid]["en_sname"] = $en_sname;
...
foreach ($aPInfo as $subj_uid => $aInfo) {
	...
	if(isset($aInfo["en_fname"])) {
		$sJS.= '$("#txtFName_en").val(getShowText("'.$aInfo["en_fname"].'"));';
		if(isset($aPOdata[$subj_uid]["en_fname"])) 
			$sJS.= '$("#txtFName_en").attr("data-odata",getShowText("'.$aInfo["en_fname"].'"));';
	}
	if(isset($aInfo["en_sname"])) {
		$sJS.= '$("#txtLName_en").val(getShowText("'.$aInfo["en_sname"].'"));';
		if(isset($aPOdata[$subj_uid]["en_sname"])) 
			$sJS.= '$("#txtLName_en").attr("data-odata",getShowText("'.$aInfo["en_sname"].'"));';
	}
	...
	/*
	if(isset($aInfo["remark"])) {
		$sJS.= '$("#txtremark").val(getShowText("'.$aInfo["remark"].'"));';
		if(isset($aPOdata[$subj_uid]["remark"])) 
			$sJS.= '$("#txtremark").attr("data-odata",getShowText("'.$aInfo["remark"].'"));';
	}*/
	if(isset($aInfo["remark"])) {
		$sJS.= '$("#txtremark").val(getShowText("'.($aInfo["remark"]).'"));';
...
<div id='divPII' class='fl-wrap-col'>
	<div class='fl-fix' style='min-height:100px;max-height:100px; text-align: center;padding:20px;<? echo(($sHideInfo=="1")?"display:none":""); ?>'>
		กรุณายืนยันข้อมูลส่วนตัว เพื่อประโยชน์ในการรักษา <br/> Please confirm your information. The data will be used for the treatment information.<br/>
		<span style='color:red'>IMPORTANT : หากท่านต้องการ ขอรับใบรับรองแพทย์ กรุณาใส่ข้อมูลที่ถูกต้อง</span>
	</div>
	<div class='fl-fill fl-auto'>
		<table id='tblPatient' cellpadding='0' cellspacing="0" style='width:100%;text-align: center' <? echo($sObjProp); ?> >
			<tr>
			...
			<tr>
				<th>อังกฤษ*<br/><span>First Name</span></th><td>:</td><td><input id='txtFName_en' name='en_fname' value='' style='width:90%' data-odata='' class='savedata checkdata' /></td>
				<th>อังกฤษ*<br/><span>Last Name</span></th><td>:</td><td><input id='txtLName_en' name='en_sname' style='width:90%' value='' data-odata='' class='savedata checkdata' /></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<th>สัญชาติ<br/><span>Nationality</span></th><td>:</td><td>
					...
					<label><input name='nation' type='radio' value='2' data-odata='' class='savedata checkdata' />อื่นๆ Other </label>
					<SELECT id='txtOtherCountry' name='country_other' class='savedata checkdata' data-odata='' ><? include("country_inc_option.php"); ?></SELECT>
					<!--input id='txtOtherCountry' data-odata='' /--></td>
				...
				<th colspan='3'>
					<div style='display:flex;vertical-align: middle'>
						...
						<div style='line-height: 40px'>
							: <input id='txtCitizenID' size='13' value='' data-odata='' class='savedata checkdata' name='citizen_id' />
						</div>
			...
			<tr style='height:30px;background-color: #dffdff'>
				...
				<th>รหัสไปรษณีย์<br/><span>Post Code</span></th><td>:</td><td><input maxlength="15" name='id_postal_code' id='txtIdPost' value='' data-odata='' class='savedata checkdata' /></td>
			</tr>
			...
			<tr id='divRemark' >
				<th colspan='9' style='text-align: center'><textarea id='txtremark' class='savedata checkdata' name='remark' style='width:99%;height:80px;<? echo(($sHideInfo=="1")?"":"display:none"); ?>'></textarea></th>
			</tr>
			<tr>
				<th colspan='9' style='text-align: center'>  <button id='btnUpdatePInfo' style='font-size:20px;<? echo(($sHideEdit=="1")?"display:none":""); ?>'>ยืนยัน / Confirm</button><img id='imgPLoader' src='assets/image/spinner.gif' style='display:none' /></th>
			</tr>
		</table>
	</div>
...
<script>
	...
	$(function(){
		...
		$("#btnUpdatePInfo").on("click",function(){
			...
			$("#tblPatient .bg-error").removeClass("bg-error");
			sSex=$("#tblPatient #ddlSex").val();
			if(sSex==""){
				$("#tblPatient #ddlSex").focus();
				$("#tblPatient #ddlSex").addClass("bg-error");
				$.notify("Please select sex at birth.\r\nกรุณาเลือกเพศกำเนิด");
				return;
			}
			sGender=$("#tblPatient #ddlGender").val();
			if(sGender==""){
				$("#tblPatient #ddlGender").focus();
				$("#tblPatient #ddlGender").addClass("bg-error");
				$.notify("Please select gender identity.\r\nกรุณาเลือกอัตลักษณ์ทางเพศ");
				return;
			}

			sPhone=$("#tblPatient #txtPhone").val();
			if(sPhone.trim()=="" || sPhone.trim().length < 10){
				$("#tblPatient #txtPhone").focus();
				$("#tblPatient #txtPhone").addClass("bg-error");
				$.notify("Please enter correct phone number.\r\nกรุณากรอกเบอร์โทรศัพท์");
				return;
			}
			...
			//Validation
			/*
			$("#txtCitizenID").css("background-color","white");
			var sId = $("#txtCitizenID").val().trim();
			if(sId!="" && sId.length!=13){
				$("#txtCitizenID").css("background-color","red");
				$.notify("เลขบัตรประชาชน 13 หลัก ไม่ถูกต้อง.");
				return;
			}
			*/
			...
			//sUrl="../weclinic/data_mgt/mnu_form_view.php?form_id="+aAllForm[0]+"&uid="+sUid+"&collect_date="+sColDate+"&collect_time="+sColTime+"&s_id=patient&next_form_id="+sNextForm;

			if(isFound==false) sUrl="";
				...
				callAjax("patient_a.php",aData,function(rtnObj,aData){
					if(rtnObj.res=="0"){
						...
						if(sAllForm==""){
							...
							$("#divPII").parent().load(sUrl,function(){});
