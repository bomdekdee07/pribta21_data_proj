...
// Qeury for check SD_ASSESSMENT_P val condition mount
...
if($sSid == ""){
	$today = date("Y-m-d");
	$bind_param = "ss";
	$array_val = array($sUid, $today);

	$query = "SELECT rs_main.collect_date,
		(select dif_value from s_d_diferent_date order by upd_date DESC limit 1) AS dif_date
	from p_data_result rs_main
	where rs_main.uid = ?
	and rs_main.collect_date <= ?
	and rs_main.data_id = 'stigma_total_score';";
	...
	if($stmt->execute()){
		...
		while($row = $result->fetch_assoc()){
			$data_coldate_main = $row["collect_date"];
			$data_dif_date = $row["dif_date"];
		}
	}
	$stmt->close();

	// create funtion calculate different date return mount
	$date_calculater = 0;

	if($data_coldate_main != ""){
		$date_calculater = calculateDateDifference($data_coldate_main, $today);
		// echo "dif cal:".$date_calculater."/ coldate:".$data_coldate_main."|".$data_dif_date;

		if($date_calculater >= $data_dif_date || $data_coldate_main == $today){
			$aFormList = array("DEMO_PRIBTA","BRA_ASSIST_PRIBTA_V2_1", "SD_ASSESSMENT_P");
		}
		else{
...
$aFormName_en[2] = "Stigma and Discrimination";
$aFormName_th[2] = "แบบประเมินการถูกตีตราและเลือกปฏิบัติ";
...
$jsHtml = ""; 
// check score form SD_ASSESSMENT_P RED
$bind_param = "sss";
...
and dt_main.data_id in ('stigma_total_score');";
$stmt = $mysqli->prepare($query);
$stmt->bind_param($bind_param, ...$array_val);
...
if($stmt->execute()){
	$result = $stmt->get_result();
	while($row = $result->fetch_assoc()){
		$data_check_score = $row["data_result"];
	}
	// echo $data_check_mental;
}
$stmt->close();

if($data_check_score >= 16){
	$jsHtml .= "$(\".btnform[data-formid='SD_ASSESSMENT_P']\").css(\"background-color\",\"#E74C3C\");";
}
