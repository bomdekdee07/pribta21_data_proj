...
if($sMode=="export_xls"){
	...
	if($gVisitId != ""){
		$bind_param .= "s";
		$array_val[] = $gVisitId;
		$sSQL_where_visitid .= " And PUV.visit_id = ? ";
	}
	...
	$query = "SELECT ".$sSQL_Log_select.",
		...
		P.gender, 
		P.date_of_birth,
		(select clinic_n.clinic_id from p_project_uid_list clinic_n where clinic_n.uid = PDR.uid and clinic_n.proj_id = PUV.proj_id limit 1) as visit_clinic_id
	...
	".$sSQL_where_visitid."
	ORDER BY ".$sSQL_Log_orderby.", PDR.uid, PDR.collect_date, PDR.collect_time, PUV.schedule_date ";
//error_log("$sFormList, $sLogList /$sProjid, $sStartD,$sStopD/ query: $query");
	// $stmt = $mysqli->prepare($query);

	// if($sFormList != "") 
	$stmt = $mysqli->prepare($query);
	$stmt->bind_param($bind_param, ...$array_val);
	// else if ($sLogList != "") $stmt->bind_param("s",$sProjid);
	// print_r($array_val);
