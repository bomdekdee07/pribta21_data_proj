...
//	error_log(print_r($lst_data));
	...
	$query = "INSERT INTO i_project_protocol (proj_id, group_id,
		...
		(?,?,?,?,?,?,?) ON Duplicate key
		UPDATE protocol_note=VALUES(protocol_note),start_date=VALUES(start_date),stop_date=VALUES(stop_date)";
		$stmt = $mysqli->prepare($query);
      ...
      $rtn['res'] = $res;
            ...
            $rtn['res'] = $res;
              ...
              WHERE proj_id=? AND group_id=? AND item_group=? AND item_group_no=?";
                ...
                $sProjid = getQS('projid');
                $txtrow = ""; $row_amt = 0;
                	$query ="SELECT proj_id,proj_id_name,proj_group_id,proj_group_name,proj_group_remark,proj_group_seq,is_disable
                ...
                //error_log()"$sProjid,$txtsearch / $query";
                	if($stmt->execute()){
                		$stmt->bind_result($proj_id,$proj_id_name,$proj_group_id, $proj_group_name, $proj_group_remark, $proj_group_seq, $is_disable);
                		while($stmt->fetch()) {
                      $res = 1;
                      $txtrow .= addRowList_proj_group($proj_group_id, $proj_id_name, $proj_group_name, $proj_group_remark, $proj_group_seq, $is_disable);
                 		}//while
                	}
                  $stmt->close();
              	$rtn['txtrow'] = $txtrow;
          ...
          //	$lst_data = getQS("lst_data");
            ...
            $query = "INSERT INTO p_project_group (proj_id, proj_id_name,proj_group_id,proj_group_name,proj_group_remark,proj_group_seq,is_disable) VALUES
              (?,?,?,?,?,?,?) ON Duplicate key
              UPDATE proj_group_name=VALUES(proj_group_name),proj_id_name=VALUES(proj_id_name),proj_group_remark=VALUES(proj_group_remark),proj_group_seq=VALUES(proj_group_seq)
               ,is_disable=VALUES(is_disable)";
              $stmt = $mysqli->prepare($query);
              $stmt->bind_param("sssssss",$projid, $lst_data['proj_id_name'], $lst_data['proj_group_id'], $lst_data['proj_group_name'],$lst_data['proj_group_remark'],$lst_data['proj_group_seq'],$lst_data['is_disable']);

              if($stmt->execute()){
                $affect_row = $stmt->affected_rows;
                $res = 1;
                $txtrow = addRowList_proj_group($lst_data['proj_group_id'], $lst_data['proj_id_name'], $lst_data['proj_group_name'],$lst_data['proj_group_remark'],$lst_data['proj_group_seq'],$lst_data['is_disable']);
              }
              else{
                error_log("proj_setting_a.php: ".$stmt->error);
              }
...
$mysqli->close();
 ...
 function addRowList_datafilter_item($item_group_no, $data_id, $data_equation, $data_value){
  ...
  function addRowList_proj_group( $proj_group_id, $proj_id_name, $proj_group_name, $proj_group_remark, $proj_group_seq,$is_disable){
   $txtrow = "
        <div class='fl-wrap-row ph50 p-row-green p-row ptxt-s10 div-setting-row ptxt-b' data-id='$proj_group_id' >
          ...
          <div class='fl-fix fl-mid pw100 s-proj_id_name' >$proj_id_name</div>
