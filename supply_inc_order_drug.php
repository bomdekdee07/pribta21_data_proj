...
$query .="  WHERE collect_date=? AND collect_time=? AND uid=? AND order_status != '00' AND supply_group_type='1'";
$stmt = $mysqli->prepare($query);
...
if($stmt->execute()){
	...
	$sHtml = ""; 
	while ($stmt->fetch()) {
		$sHtml .= getSupOrderDrug($order_id,$supply_code,$supply_name,$supply_unit,$dose_day,$dose_per_time,$dose_before,$dose_breakfast,$dose_lunch,$dose_dinner,$dose_night,$order_note,$sale_opt_id,$stock_lot,$sale_price,$total_amt,$order_status,$supply_group_type,$supply_desc,$s_name);
	}
}
$mysqli->close();
echo($sHtml);
