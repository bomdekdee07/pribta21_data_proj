...
if($sMode=="q_create"){
			...
			if($stmt->execute()){
				...
				if($iAffRow > 0) {
					...
					if(isset($_GET["notetoall"]) || isset($_POST["notetoall"])){
						...
						$iNoteRow=0;
						$stmt->bind_param("sssss",$sNote,$sSid, $sProjId, $sClinicId,$sToday,$sQ);
						if($stmt->execute()){
	...
	if($isCont){
		//Check if Q is already bind with other? or exist
		//$isCont = false;
		$query = "SELECT IQL.uid,PDR.uid,PLO.uid,ISO.uid FROM i_queue_list IQL
		...
		$stmt->bind_param('ssss',$sQ,$sUid,$sColD,$sClinicId);
		$stmt->execute();
		$stmt->bind_result($iql,$pdr,$plo,$iso);
		$iCount=0;
		while ($stmt->fetch()) {
			if($pdr!=""){
