...
if($sRoom!=""){
	...
	$query="SELECT IRL.room_no,room_detail,s_name,IRLIST.s_id FROM i_room_list IRL 
	...
	if($stmt->execute()){
	  $stmt->bind_result($room_no,$room_detail,$s_name,$s_id);
	  while ($stmt->fetch()) {
	  	$sHtml.=getRoomListRow($room_no,$room_detail,$s_id,$s_name);
	  }
	}
}
