<?
    ...
    if($stmt->execute()){
        ...
        while($stmt->fetch()){
            if($is_service == 1){
                ...
                $data_servcie_loop[$supply_code][$sex]["price"] = $total_price_sum;
                ...
                $total_price_sum += $total_price;
                $data_servcie_loop[$supply_group_type][$sex]["price"] = $total_price_sum;
    ...
    if($stmt->execute()){
        ...
        while($stmt->fetch()){
            ...
            $data_servcie_loop[$lab_id][$sex]["price"] = $total_price_sum;
    ...
    if($start_date == "")
        ...
        $genStName = $start_date."-".$end_date;
    $file_name = "Report_Monthly_Service_".$genStName.".xlsx";
    ...
    foreach($data_servcie_loop as $key_supcode => $data_sex){
        foreach($data_sex as $key_sex => $values){
            $arrayTemp = array();
