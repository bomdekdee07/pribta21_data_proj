...
$queryX = "INSERT INTO p_data_result (uid,collect_date,collect_time,data_id, data_result, lastupdate, s_id, proj_id)
...
foreach($lst_data as $itm){
  ...
  if($res){
    ...
    $data_result = urldecode($arr_data[2]);
      $stmtX->bind_param("sssssss", $data_id, $data_result,$s_id, $projid, $uid, $formid, $row_id );
      if($stmtX->execute()){
        $affect_row = $stmtX->affected_rows;
        if($affect_row > 0){
          $res = 1;
          addToLog("update log [$uid|$formid|$row_id] $data_id:$data_result"." Project:".$projid, $s_id);
        }else{
          $res = 0;
