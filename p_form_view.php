  ...
  while ($stmt->fetch()) {
    if (empty($data_type)) {
      ...
      if($data_type == "dropdown"){
        ...
        $class_sumscore = "";
        if(isset($d_itm_prop[$data_id]['sumscore'])){
          if($d_itm_prop[$data_id]['sumscore'] == "1"){
            $class_sumscore = "v-sumscore";
          }
        }

        $d_sub_item[$data_id] .= "<label class='pbtn' $radio_css><input type='radio' class='save-data v-radio $class_sumscore'  name='$data_id' data-id='$data_id' data-isrequire='$is_require' data-odata='' value='$data_value' /> <span class='lang th'>$data_name_th</span><span class='lang en'>$data_name_en</span> </label>$radio_html";
      }
...
// $stmt->bind_param('sssss',$uid, $collect_date, $collect_time, $form_id, $sProjid);
//echo "query : $query";
    ...
    if($data_type == "html"){ // html
      ...
      if($data_type == "text" || $data_type == "date"){
        if($data_type == "date"){
          ...
          // not exceed collect date BOM 11/05/2023
          if(isset($d_itm_prop[$data_id]['notexceedcoldate'])){
            if($d_itm_prop[$data_id]['notexceedcoldate'] == '1'){
              $data_type = "date-notexceed";
              $special_data_notexceed = "date-notexceed";
            }
          }
        }//date
        $sHtml .= " <INPUT type='text' name='$data_id' value='$data_result' data-id='$data_id' data-odata='$odata' data-isrequire='$is_require' data-coldate='$collect_date' class='save-data $color_require v-$data_type $special_data_class $special_data_notexceed' style='$width' $placeholder>";
      }
      else if($data_type == "number"){
        $class_sumscoretotal = "";
        if(isset($d_itm_prop[$data_id]['sumscoretotal'])){
          if($d_itm_prop[$data_id]['sumscoretotal'] == "1"){
            $class_sumscoretotal .= "v-sumscoretotal";
          }
        }
        $sHtml .= " <INPUT type='number' name='$data_id' value='$data_result' data-id='$data_id' data-odata='$odata' data-isrequire='$is_require' class='save-data $color_require v-$data_type $special_data_class $class_sumscoretotal' style='$width' $placeholder>";
      }
...
if($collect_time == '00:00:00'){
  ...
  ";
  //AND PDR.proj_id = ? wait edit all code

  $stmt = $mysqli->prepare($query);
  //$stmt->bind_param('ssss',$uid,$collect_date,$form_id, $sProjid);
  ...
  <div class='fl-wrap-col fl-auto'>
    <div class='fl-wrap-row fl-fix ph25'>
      ...
      <div class='fl-wrap-col fl-fix pw100 ' >
          <select id='sel_form_lang' name='sel_form_lang' class='ml-1 ptxt-8' style='width:80px;'><option value='th'>ไทย</option><option value='en'>English</option></select>
      </div>
    </div>
...
<script>
var check_click_form = 0;

$(".div-form-view").off("off");
$(".div-form-view").on("click", function(){
  check_click_form += 1;

  if(check_click_form == 1){
    var objAlert = undefined;
    $(".div-form-view .save-data").each(function(ix,objx){
      ...
      if($(objx).attr('data-isrequire') == '1' && $(objx).is(':visible')){
        ...
        if($(objx).prop("type").toLowerCase()=="checkbox"){
          ...
          if($('input[name="'+q_label_name+'"]:checked').length > 0){}
          else objAlert = objx; 
        }
    ...
    // var condition_process = element_array.length - element_array_checked.length;
    var form_id_click = $(this).attr("data-formid");
    let sUID = $(this).attr('data-uid');
    let sVisitid = $(this).attr('data-visitid');
    let sColdate = $(this).attr('data-coldate');
    let sColtime = $(this).attr('data-coltime');
    // // console.log(form_id_click+"/"+condition_process);

    if(objAlert != undefined){
      // loop change color
      $.each(window.parent.$("#qa_data_defult .btnform"), function(){
        // console.log($(this).attr("data-formid"));
        if($(this).attr("data-formid") == form_id_click){
          $(this).attr("style", "");
        }
      });
      ...
      var aData = {
        ...
        coldate: sColdate,
        coltime: sColtime,
        formid: form_id_click,
        isdone: sIsdone
      };
      // console.log(aData);

      $.ajax({
        url: "p_form_view_ajax_check_form_done.php",
        type: "POST",
        cache: false,
        data: aData,
        success: function(sResult){
          // console.log(sResult);
          if(sResult == "complete"){
            console.log("update done:"+sIsdone);
          }
        }
      }); 
    }
    else{
      console.log("require:"+$(objAlert).attr("data-id")+"/"+objAlert);
      // loop change color
      $.each(window.parent.$("#qa_data_defult .btnform"), function(){
        // console.log($(this).attr("data-formid"));
        if($(this).attr("data-formid") == form_id_click){
...
$(document).ready(function(){
  ...
  if($(".div-form-view").attr("data-lang") != ''){
    ...
    if(localStorage['ss_language'] !== undefined && localStorage['uid'] == sUID){
      if(localStorage['ss_language'] == "en"){
      ...
      $("#sel_form_lang").val(localStorage['ss_language']);
    }
    else{
      $.datepicker.setDefaults($.datepicker.regional['th']);
      $("#sel_form_lang").val("th");
    }
  }
  else{
    $.datepicker.setDefaults($.datepicker.regional['th']);
  ...
  $(".div-form-view #sel_form_lang").on("change",function(){
    // console.log("IN:"+localStorage['ss_language']);

    let lang = $(this).val();
    var ss_language = localStorage['ss_language'] = lang;
    ...
    let sUID = $('.div-form-view').attr('data-uid');
    localStorage['uid'] = sUID;

    // console.log("LAST:"+ss_language);

    $('.div-form-view .lang').hide();
    $('.div-form-view .'+ss_language).show();

    //dropdownlist change language
    $( ".div-form-view .ddl").each(function() {
        $(this).text($(this).attr('data-val'+ss_language));
    });

    $(".div-form-view").attr("data-lang", '"'+ss_language+'"')
    // console.log($(".div-form-view").attr("data-lang"));
  });
  ...
  $(".div-form-view .btn-hist-log").on("click",function(){
    ...
    let sUrl = "p_form_history_log.php?uid="+sUID+"&coldate="+sColdate+"&coltime="+sColtime+"&formid="+sFormid+"&proj_id="+sProjid;
    showDialog(sUrl,"Data History | Form ID: "+sFormid+ " ["+sProjid+"][UID:"+sUID+" | Date:"+sColdate+" "+sColtime+"]","90%","99%","",function(sResult){
  			if(sResult != ""){
  			}
  	},false,function(){
  	});
...
*/
  ...
  $(".div-form-view").on("change",".v-radio",function(){
    ...
    // Add ATB sum score
    var total_score_atb = 0;
    var count_check_val_atb = 0;
    $(".div-form-view .v-sumscore").filter(":checked").each(function(){
      if($.isNumeric($(this).filter(":checked").val())){
        total_score_atb += parseInt($(this).val());
      }
      else{
        total_score_atb += 0;
      }
      ...
      var check_total_val = $(".div-form-view .v-sumscoretotal").length;
      if(check_total_val > 0){
        $(".div-form-view .v-sumscoretotal").val(total_score_atb);
      }
  ...
  // not exceed collect date
  $('.div-form-view').on('click', '.date-notexceed', function(){
    if($(this).hasClass('hasDatepicker')){
    }
    else{
  ...
  $('.div-form-view').off('keyup');
  $('.div-form-view').on('keyup', '.date-notexceed', function(e){
    var d = new Date();
    var year_cerrnt = d.getFullYear();
    var month = d.getMonth()+1;
    var day = d.getDate();
    var date_current = d.getFullYear() + '-' +(month<10 ? '0' : '') + month + '-' +(day<10 ? '0' : '') + day;

    if (e.keyCode != 8){  
      if ($(this).val().length == 4){
          if($(this).val() <= (year_cerrnt+477)){
            $(this).val($(this).val() + "-");
          }
          else{
            var this_del = $(this).val().slice(0,-4);
            $(this).val(this_del);
            alert("กรุณาพิมพ์ปีรูปแบบ คศ.");
          }
      } 
          ...
          var this_del = $(this).val().slice(0,-1);
          $(this).val(this_del);
      }

      var coldate = new Date($(this).attr("data-coldate"));
      var txt_coldate = $(this).attr("data-coldate");
      var val_split = $(this).val().split("-");
      var val_current_split = txt_coldate.split("-");

      if(val_split[1] != "00" && val_split[2] != "00"){
        var date_this = new Date($(this).val());

        var date_diff = date_this - coldate;
        date_diff = date_diff/1000/60/60/24;

        if(date_diff > 0){
          var this_del = $(this).val().slice(0,-10);
          $(this).val(this_del);
          alert("ห้ามเกินวันที่เข้ารับบริการ");
        }
      }
      else{
        if(val_split.length >= 3){
          if(val_split[1] != "00" && val_split[2] == "00"){
            if(val_split[0] > val_current_split[0]){
              var this_del = $(this).val().slice(0,-10);
              $(this).val(this_del);
              alert("ห้ามเกินวันที่เข้ารับบริการ");
            }
            else{
              if(val_split[1] > val_current_split[1] && val_split[0] >= val_current_split[0]){
                var this_del = $(this).val().slice(0,-5);
                $(this).val(this_del);
                alert("ห้ามเกินวันที่เข้ารับบริการ");
              }
            }
          ...
          // console.log("in:"+val_split+"/"+val_current_split);
        }
      }
    } 
    else{
