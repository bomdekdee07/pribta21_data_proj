<?
    ...
    AND data_re.data_id = 'cn_patient_note' AND (queue_l.sale_opt_id = 'R04' OR data_re.data_result LIKE '%083%') ";
    ...
    if($stmt->execute()){
        ...
        while ($stmt->fetch()) {
            $data_htpn083_uid[$uid.$queue_datetime]["uid"] = $uid;
            $data_htpn083_uid[$uid.$queue_datetime]["date"] = $queue_datetime;
            $data_htpn083_uid[$uid.$queue_datetime]["sale"] = $sale_opt_id;
            $data_htpn083_uid[$uid.$queue_datetime]["note"] = $note_to_all;
            $data_htpn083_uid[$uid.$queue_datetime]["queue"] = $queue;
            if(isset($fname)){
            ...
            $data_htpn083_uid[$uid.$queue_datetime]["name_info"] = $name_info;
        }
        // print_r($data_htpn083_uid);
    }
    ...
    foreach($data_htpn083_uid as $key => $values){
        $arrayTemp = array();
