...
//Load queue info
$query ="SELECT queue,IQL.uid,collect_date,collect_time,queue_status,queue_call,queue_note,room_no,prepare_drug_by,check_drug_by,issue_drug_by,receive_by,queue_type,prepare_drug_date,check_drug_date,issue_drug_date,IBD.bill_id, IQL.queue,IQL.uid,IQL.collect_date,IQL.collect_time,IQL.queue_status,IQL.queue_call,IQL.queue_note,IQL.room_no,IQL.prepare_drug_by,IQL.check_drug_by,IQL.issue_drug_by,IBL.receive_by,IQL.queue_type,IQL.prepare_drug_date,IQL.check_drug_date,IQL.issue_drug_date,IBD.bill_id, (select billcan.status from i_bill_cancel_approve billcan where billcan.uid = IQL.uid and billcan.collect_date = IQL.collect_date and billcan.collect_time = IQL.collect_time order by date_now limit 1) as status_billcan
...
if($stmt->execute()){
  ...
  while($row = $result->fetch_assoc()) {
	...
	if($aQInfo["collect_date"] != $sToday && $aQInfo["status_billcan"] == ""){
		//Archive History
		$btnSaveMode="archive";
...
<div id='divPharIncM' class='fl-wrap-col' data-uid='<? echo($sUid); ?>' data-coldate='<? echo($sColD); ?>' data-coltime='<? echo($sColT); ?>'>
	...
	<div class='fl-wrap-row h-30 lh-20 fs-small bg-head-1'>
		...
		<!-- ตารางนัดคนไข้ -->
		<div class="fl-fix fl-mid" id="appointments_schedule">
