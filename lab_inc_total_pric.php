<?
    ...
    $query = "SELECT lab_test.lab_name,
        ...
        lab_order_lab_test.sale_price,
        lab_order_lab_test.lab_id,
        lab_rs.lab_result_status
    from p_lab_order lab_order
    left join p_lab_order_lab_test lab_order_lab_test on(lab_order_lab_test.uid = lab_order.uid and lab_order_lab_test.collect_date = lab_order.collect_date and lab_order_lab_test.collect_time = lab_order.collect_time)
    left join p_lab_test lab_test on(lab_test.lab_id = lab_order_lab_test.lab_id)
    left join p_lab_test_group lab_group on(lab_group.lab_group_id = lab_test.lab_group_id)
    left join p_lab_result lab_rs on(lab_rs.uid = lab_order.uid and lab_rs.collect_date = lab_order.collect_date and lab_rs.collect_time = lab_order.collect_time and lab_rs.lab_id = lab_order_lab_test.lab_id)
    where lab_order.lab_order_status != 'C'
    and lab_order.uid = ?
    and lab_order.collect_date = ?
    and lab_order.collect_time = ?
    ...
    if($stmt->execute()){
        $stmt->bind_result($lab_name, $lab_group_name, $sale_price, $lab_id, $lab_result_status);
        while ($stmt->fetch()) {
            $lab_saleprice_data2[$lab_id]["lab_name"] = $lab_name;
            $lab_saleprice_data2[$lab_id]["total_price"] = $sale_price;
            $lab_saleprice_data2[$lab_id]["result_status"] = $lab_result_status;
            $total_price_sum_lab_all2 = $total_price_sum_lab_all2+$sale_price;
        }
        // print_r($lab_saleprice_data2);
    }
    ...
    if(count($lab_saleprice_data2) > 0){
        foreach($lab_saleprice_data2 as $key => $val){
            $title_icon = "ผลแล๊บอยู่ในขั้นตอนการออกผล / Internal lab is on process.";
            $color_icon = "orange";
            if($val["result_status"] != ""){
                $color_icon = "green";
                $title_icon = "ผลแล๊บเสร็จสิ้น / Internal lab is complete.";
            }
            ...
            $SJS_group .=   '<div class="fl-wrap-row h-35 fs-smaller row-color">
                                ...
                                <div class="fl-fix wper-5 fl-mid fw-b" title="'.$title_icon.'">
                                    <i class="fa fa-vial" style="color:'.$color_icon.'"></i>
                                </div>
                            </div>';
        }                            
    }                        
    ...
    $SJS_group .=       '
                        <div class="fl-wrap-row fw-b border h-30 smallfont3" style="background-color: #68C95C;">
                            <div class="fl-fix wper-5 fl-mid"></div>
                            <div class="fl-fix wper-55 fl-mid-left">
                                <span>ยอดรวม</span>
                            </div>
