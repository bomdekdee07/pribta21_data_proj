...
if($sFind=="%%"){
	...
	UNION
	SELECT '0', '1', 'Package items', package_stock_id, package_stock_name, 'Package items group', '', '', '0', '', '1', '0000-00-00'
	...
	if($stmt->execute()){
		...
		while($stmt->fetch()){
			$sHtml .= getSupplySearchRow($sShowAmt,$is_service,$supply_group_type,$supply_group_name,$supply_code,$supply_name,$supply_desc,$supply_unit,$stock_lot,$stock_amt,$stock_exp_date,$bulk_unit,$convert_amt);
		}
	}
	if($sHtml=="") $sHtml="ไม่พบข้อมูล ภายใต้สิทธิ์ของท่าน<br/>No data found under your authorization.<br/>".$sFind ." ";
