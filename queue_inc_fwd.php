...
$section_array = array();
$section_function = ["D03"=>"pharmacy", "D05"=>"physician", "D06"=>"physician", "D07"=>"cashier"]; 

$query = "SELECT sc_id.section_id,
	sc_name.section_name
from p_staff_clinic sc_id
join p_staff_section sc_name on(sc_name.section_id = sc_id.section_id)
where sc_id.s_id = ?;";
...
$count_check = 0;
$aready_function_name = "";
foreach($section_array as $val){
	if(isset($section_function[$val])){
		$count_check++;
	}
}
...
if($count_check > 0){
	if($count_check == 1){
		$aready_function_name = $section_function[$section_array[0]];
	}
...
// echo "NAME:".$aready_function_name."/".$count_check;
...
<div id='divQueueFwd' class='fl-wrap-col' data-projid='<? echo $projid; ?>' data-section='<?echo $aready_function_name; ?>'>
...
<script>
	$(document).ready(function(){
		...
		$("#divQueueFwd #btnUidFwd").on("click",function(){
			...
			var sprojid = $("#divQueueFwd").data("projid");
			var sSectionName = $("#divQueueFwd").data("section");
			// console.log("btnUidFwd:"+sRoomNo);

			if(sRoomNo == "28"){
				sSaleId = $("#ddlSaleOpt").val();
				sQ =$("#divQueueFwd #txtFwdQueue").val();
				var sUrl_appoint = "queue_inc_fwd_alert_home.php?u_mode=q_fwd&q="+sQ+"&fwdroom=28&saleid="+sSaleId+"&projid="+sprojid+"&section_name="+sSectionName;

				showDialog(sUrl_appoint, "ยืนยันการส่งกลับบ้าน", "70%", "80%", "", function(sResult){
					var obj_this = $("#divQueueFwd #btnUidFwd");
					close_dlg(obj_this);
